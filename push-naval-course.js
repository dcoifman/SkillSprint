...
async function createInstructor() {
  const instructor = {
    ...
    bio: 'Expert in naval history and military disasters, with a special focus on the Russo-Japanese War.',
...
async function pushNavalCourse() {
  try {
    // Read the course data
    const courseData = JSON.parse(
      await fs.readFile(join(__dirname, 'funny_naval_disaster_course.json'), 'utf8')
    );
    ...
    if (existingPath) {
      ...
      const { error: updateError } = await supabase
        .from('learning_paths')
        .update({ image: '/path-images/russian-baltic-fleet.jpg' })
    ...
    console.log('Successfully pushed naval course with all components');
  } catch (error) {
