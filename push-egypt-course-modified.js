...
const supabaseServiceRoleKey = process.env.SUPABASE_SERVICE_ROLE_KEY || process.env.REACT_APP_SUPABASE_SERVICE_ROLE_KEY;

if (!supabaseUrl || !supabaseServiceRoleKey) {
  throw new Error('REACT_APP_SUPABASE_URL and SUPABASE_SERVICE_ROLE_KEY (or REACT_APP_SUPABASE_SERVICE_ROLE_KEY) environment variables are required');
}
      ...
      for (let j = 0; j < moduleData.sprints.length; j++) {
        ...
        const { error: sprintError } = await supabase
          ...
          .insert({
            ...
            content: sprintData.contentOutline,
            order_index: j + 1
          });
